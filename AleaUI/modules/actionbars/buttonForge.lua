...
BF.eventFrame = CreateFrame("Frame", 'AleaUIButtonForgeModuleEventFrame')
BF.eventFrame:UnregisterAllEvents()

BF.buttonRegistry = BF.buttonRegistry or {}
BF.activeButtons = BF.activeButtons or {}
BF.actionButtons = BF.actionButtons or {}
BF.nonActionButtons = BF.nonActionButtons or {}

BF.ChargeCooldowns = BF.ChargeCooldowns or {}
BF.NumChargeCooldowns = BF.NumChargeCooldowns or 0
BF.TotalChargeCooldowns = BF.TotalChargeCooldowns or {}

BF.ACTION_HIGHLIGHT_MARKS = BF.ACTION_HIGHLIGHT_MARKS or setmetatable({}, { __index = ACTION_HIGHLIGHT_MARKS })

--BF.callbacks = BF.callbacks or CBH:New(BF)
...
local ButtonRegistry, ActiveButtons, ActionButtons, NonActionButtons = BF.buttonRegistry, BF.activeButtons, BF.actionButtons, BF.nonActionButtons

local actionMap = BF.actionMap
...
function BF:CreateButton(id, name, header, config)
...
--	BF.callbacks:Fire("OnButtonCreated", button)
...
--	BF.callbacks:Fire("OnButtonContentsChanged", self, state, self.state_types[state], self.state_actions[state])
...
function Generic:PostClick()
	...
	if self._state_type == "action" and BF.ACTION_HIGHLIGHT_MARKS[self._state_action] then
		ClearNewActionHighlight(self._state_action, false, false)
	end
end
...
function InitializeEventHandler()
	BF.eventFrame:SetScript("OnEvent", OnEvent)
	BF.eventFrame:RegisterEvent("PLAYER_ENTERING_WORLD")
	BF.eventFrame:RegisterEvent("ACTIONBAR_SHOWGRID")
	BF.eventFrame:RegisterEvent("ACTIONBAR_HIDEGRID")
	BF.eventFrame:RegisterEvent("PET_BAR_SHOWGRID")
	BF.eventFrame:RegisterEvent("PET_BAR_HIDEGRID")
	--BF.eventFrame:RegisterEvent("ACTIONBAR_PAGE_CHANGED")
	--BF.eventFrame:RegisterEvent("UPDATE_BONUS_ACTIONBAR")
	...
	BF.eventFrame:RegisterEvent("ACTIONBAR_UPDATE_STATE")
	BF.eventFrame:RegisterEvent("ACTIONBAR_UPDATE_USABLE")
	BF.eventFrame:RegisterEvent("ACTIONBAR_UPDATE_COOLDOWN")
	...
	BF.eventFrame:RegisterEvent("SPELL_UPDATE_COOLDOWN")
	BF.eventFrame:RegisterEvent("SPELL_UPDATE_USABLE")
	BF.eventFrame:RegisterEvent("PLAYER_EQUIPMENT_CHANGED")

	BF.eventFrame:RegisterEvent("LOSS_OF_CONTROL_ADDED")
	BF.eventFrame:RegisterEvent("LOSS_OF_CONTROL_UPDATE")

	BF.eventFrame:Show()
	BF.eventFrame:SetScript("OnUpdate", OnUpdate)
end
...
--	BF.callbacks:Fire("OnKeybindingChanged", self, key)
end
...
--	BF.callbacks:Fire("OnKeybindingChanged", self, nil)
end
...
--	BF.callbacks:Fire("OnButtonUpdate", self)
end
...
--	BF.callbacks:Fire("OnButtonState", self)
end
...
--	BF.callbacks:Fire("OnButtonUsable", self)
end
...
function EndChargeCooldown(self)
	...
	tinsert(BF.ChargeCooldowns, self)
end

local function StartChargeCooldown(parent, chargeStart, chargeDuration)

	if not parent.chargeCooldown then
		local cooldown = tremove(BF.ChargeCooldowns)
		if not cooldown then
			BF.NumChargeCooldowns = BF.NumChargeCooldowns + 1
			cooldown = CreateFrame("Cooldown", "AleaUIChargeCooldown"..BF.NumChargeCooldowns, parent, "CooldownFrameTemplate");
		...
		--	cooldown:SetEdgeTexture("Interface\\Cooldown\\edge", 1, 1, 1, 1);
			--edgeBlackAndWhite
			--edgeBlackAndWhite2
			--edgeWhite3
			cooldown:SetEdgeTexture(E:GetEdgeTexture()) --, 255/255, 200/255, 0, 0.8);
			
		--	cooldown.SizeOverride = 0.6
		--	E:RegisterCooldown(cooldown)
		
			BF.TotalChargeCooldowns[#BF.TotalChargeCooldowns+1] = cooldown
		end
...
function ClearNewActionHighlight(action, preventIdenticalActionsFromClearing, value)
	BF.ACTION_HIGHLIGHT_MARKS[action] = value
	...
	for actionKey, markValue in pairs(BF.ACTION_HIGHLIGHT_MARKS) do
		if markValue then
...
hooksecurefunc("MarkNewActionHighlight", function(action)
	BF.ACTION_HIGHLIGHT_MARKS[action] = true
	for button in next, ButtonRegistry do
		if button._state_type == "action" and action == tonumber(button._state_action) then
			UpdateNewAction(button)
		end
